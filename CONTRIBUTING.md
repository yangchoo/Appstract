...
This is a community-maintained fork of [Melanie Gutzmann's](https://github.com/mirrorkeydev) original Appstract, rebuilt for F-Droid. The icons were designed by Melanie; this fork keeps the pack alive and accepting contributions. Questions or proposals are welcome in the [issue tracker](https://github.com/yangchoo/Appstract/issues).

## Icons

You're welcome to design icons for apps you'd like to see supported by Appstract.

Don't know what to add, but want to help? See Appstract's [most-requested icons](https://github.com/yangchoo/Appstract/blob/master/icons/most-requested-icons.txt). You'll want to verify that these icons don't already exist - [`drawable.xml`](https://github.com/yangchoo/Appstract/blob/master/app/src/main/res/xml/drawable.xml) has a list of all icons that have been created. You're also welcome to redesign existing icons if you don't like the way they look, but be aware that redesigns will have to hold up to a higher level of scrutiny than adding new icons.
...
Have a question about any of this or any of your icons, but don't want to open a pull request yet? Open an [issue](https://github.com/yangchoo/Appstract/issues) and ask there.
...
3. Export the icons as 192x192px .PNGs to the [`appstract-light` folder](https://github.com/yangchoo/Appstract/tree/master/icons/appstract-light) with black lines, and to the [`appstract-dark` folder](https://github.com/yangchoo/Appstract/tree/master/icons/appstract-dark) with white lines. These should have the same name as the svg (e.g. `clash_of_clans.svg` -> `clash_of_clans.png`).
4. Add the activity names of the icons you've added to `app/src/main/res/xml/appfilter.xml`. For more information, see [Activity Names](#Activity-Names).
5. Add yourself to the `app/src/main/res/xml/contributors.xml` file.
...
Don't worry about adding your icon to `app/src/main/res/xml/drawable.xml`; that file is autogenerated based on the `appstract-light` folder.
...
Once you've opened the PR, you might see a button that says "Comment and Close". Closing the PR means your changes won't be reviewed. Make sure you don't press the "Comment and Close" button unless that's what you want to happen; use the "Comment" button instead.
9. Once a maintainer approves the icons, the pull request will be merged (your changes added to the main repository).

If at any point you're having trouble with any of the steps, feel free to open an [issue](https://github.com/yangchoo/Appstract/issues) :)
...
- If your icon is in the list of [most-requested icons](https://github.com/yangchoo/Appstract/blob/master/icons/most-requested-icons.txt), you should be able to find activity names there.
...
Feel free to make changes to documentation files. If you want to propose a large, restructuring change to these docs, or even just discuss a smaller one, feel free to open an [issue](https://github.com/yangchoo/Appstract/issues).
...
Ideas:
- A workflow that checks that all lines in the svgs provided are 6px wide. (This is tricky - vector matrix transformation stuff makes it hard to tell what a line width is just based on the svg xml. It'll say 16.299382 in the file and it'll be 6px in Inkscape.)
